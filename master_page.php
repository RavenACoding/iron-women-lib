...
<body>
    ...
    <!-- MAIN CONTENT --->
    <main class="main">
        <div class="main-top"><h1><?= t('Welcome', 'Bienvenido') ?>, <span class="highlight"><?= htmlspecialchars($currentUser['first_name']) ?></span></h1></div>
        <section id="dashboard-section" class="content-section active">
            <div class="stats-grid">
                <div class="stat-card"><h3><?= $totalUsers ?></h3><p><?= t('Total Users', 'Usuarios') ?></p></div>
                <div class="stat-card"><h3><?= $totalAdmins ?></h3><p><?= t('Total Admins', 'Admins') ?></p></div>
                <div class="stat-card"><h3><?= $totalIncidents ?></h3><p><?= t('Incidents', 'Incidentes') ?></p></div>
                <div class="stat-card"><h3><?= $totalCerts ?></h3><p><?= t('Certificates', 'Certificados') ?></p></div>
            </div>
        ...
        <section id="users-section" class="content-section">
            <div class="section-header"><h2>
                <?= t('User Management', 'Usuarios') ?></h2><input type="text" id="userSearch" placeholder="
            ...
            <div class="card">
                <table id="usersTable">
                    <thead><tr><th>
                        <?= t('Name', 'Nombre') ?></th><th>
                        <?= t('Email', 'Correo') ?></th><th>
                        <?= t('Dept', 'Depto') ?></th><th>
                        <?= t('Sub-Dept', 'Sub-Depto') ?></th><th><?= t('Status', 'Estado') ?></th><th>
                        <?= t('Actions', 'Acciones') ?></th></tr></thead>
                    <tbody>
                        <?php $allUsers->data_seek(0); while ($user = $allUsers->fetch_assoc()): ?>
                        ...
                        <tr class="<?= $user['is_active'] == 0 ? 'inactive' : '' ?>">
                            ...
                            <td><span class="status-badge status-<?= $user['is_active'] == 1 ? 'active' : 'inactive' ?>">
            ...
            <div class="section-header">
                ...
                <div class="card">
                    ...
                    <div class="incident-item">
                        ...
                        <p>
                            <?= t('User', 'Usuario') ?>: <?= htmlspecialchars($inc['user_name']) ?> | <?= t('Date', 'Fecha') ?>: <?= $inc['incident_date'] ?>
                        </p>

                        <?php if ($inc['notes']): ?><p class="notes">
                        <?= t('Notes', 'Notas') ?>: <?= htmlspecialchars($inc['notes']) ?></p><?php endif; ?>
                    </div>
                </div>
                <?php endwhile; ?>
            </div>
        ...
        <section id="certificates-section" class="content-section">
            ...
            <div class="card"><div class="cert-grid">
                <?php $certificates->data_seek(0); while ($cert = $certificates->fetch_assoc()): $expiry = new DateTime($cert['expiry_date']); $today = new DateTime(); $diff = $today->diff($expiry)->days; $isExpired = $today > $expiry; $isExpiring = !$isExpired && $diff <= 30; $statusClass = $isExpired ? 'expired' : ($isExpiring ? 'expiring' : 'valid'); ?>
        ...
        <section id="profile-section" class="content-section">
            ...
            <div class="card"><div class="profile-grid">
                <div class="profile-item"><strong><?= t('First Name', 'Nombre') ?></strong><span><?= htmlspecialchars($currentUser['first_name']) ?></span></div>
                <div class="profile-item"><strong><?= t('Middle Name', 'Segundo') ?></strong><span><?= htmlspecialchars($currentUser['middle_name'] ?? '') ?></span></div>
                <div class="profile-item"><strong><?= t('Last Name', 'Apellido') ?></strong><span><?= htmlspecialchars($currentUser['last_name']) ?></span></div>
                <div class="profile-item"><strong><?= t('Email', 'Correo') ?></strong><span><?= htmlspecialchars($currentUser['email']) ?></span></div>
                <div class="profile-item"><strong><?= t('Phone', 'Telefono') ?></strong><span><?= htmlspecialchars($currentUser['phone'] ?? 'N/A') ?></span></div>
                <div class="profile-item"><strong><?= t('Department', 'Depto') ?></strong><span><?= htmlspecialchars($currentUser['department'] ?? 'N/A') ?></span></div>
            </div></div>
        </section>
        <!-- SETTINGS SECTION --->
    ...
    <div id="editUserModal" class="modal">
        <div class="modal-content">
            ...
            <form id="editUserForm" onsubmit="saveUserEdit(event)">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select id="editSubDept" onchange="toggleNewSubDept()">
                            ...
                            <?php foreach ($existingSubDepts as $sd): ?><option value="<?= htmlspecialchars($sd) ?>"><?= htmlspecialchars($sd) ?></option><?php endforeach; ?>
                ...
                <div class="form-group" id="assignAdminGroup"><label><?= t('Assign Admin', 'Asignar') ?></label><select id="editAssignedAdmin"><option value=""><?= t('Unassigned', 'Sin asignar') ?></option><?php $regularAdmins->data_seek(0); while ($ra = $regularAdmins->fetch_assoc()): ?><option value="<?= $ra['id'] ?>"><?= htmlspecialchars($ra['name']) ?><?= $ra['sub_department'] ? ' (' . htmlspecialchars($ra['sub_department']) . ')' : '' ?></option><?php endwhile; ?></select></div>
    ...
    <div id="incidentModal" class="modal">
        <div class="modal-content">
            ...
            <form id="incidentForm" onsubmit="submitIncident(event)">
                <div class="form-group"><label><?= t('Select User', 'Usuario') ?> *</label><select id="incidentUserId" required><option value="">--</option><?php $allUsers->data_seek(0); while ($u = $allUsers->fetch_assoc()): ?><option value="<?= $u['id'] ?>"><?= htmlspecialchars($u['name']) ?></option><?php endwhile; ?></select></div>
    ...
    <div id="certModal" class="modal">
        <div class="modal-content">
            ...
            <form id="certForm" onsubmit="submitCertificate(event)">
                ...
                <div class="form-group"><label><?= t('Select Users', 'Usuarios') ?></label><div id="certUserList" class="user-checkbox-list"><?php $allUsers->data_seek(0); while ($u = $allUsers->fetch_assoc()): ?><label class="checkbox-item"><input type="checkbox" name="certUsers[]" value="<?= $u['id'] ?>"><?= htmlspecialchars($u['name']) ?></label><?php endwhile; ?></div></div>
    ...
    <div id="profileModal" class="modal">
        <div class="modal-content">
            ...
            <form id="profileForm" onsubmit="saveProfile(event)">
                ...
                <div class="form-row"><div class="form-group"><label><?= t('First', 'Nombre') ?></label><input type="text" id="profileFirstName" value="<?= htmlspecialchars($currentUser['first_name']) ?>" required></div><div class="form-group"><label><?= t('Middle', 'Segundo') ?></label><input type="text" id="profileMiddleName" value="<?= htmlspecialchars($currentUser['middle_name'] ?? '') ?>"></div><div class="form-group"><label><?= t('Last', 'Apellido') ?></label><input type="text" id="profileLastName" value="<?= htmlspecialchars($currentUser['last_name']) ?>" required></div></div>
                <div class="form-group"><label><?= t('Email', 'Correo') ?></label><input type="email" id="profileEmail" value="<?= htmlspecialchars($currentUser['email']) ?>" required></div>
                <div class="form-group"><label><?= t('Phone', 'Telefono') ?></label><input type="text" id="profilePhone" value="<?= htmlspecialchars($currentUser['phone'] ?? '') ?>"></div>
                ...
                foreach ($departments as $dept): 
                    ...
                    if ($deptAdminCount > 1):
                        ...
                        while ($admin = $deptAdminsQuery->fetch_assoc()):
                            $assignedCount = $conn->query("SELECT COUNT(*) as c FROM users WHERE assigned_admin = {$admin['id']}")->fetch_assoc()['c'];
                ?>
                <div class="dept-card" onclick="location.href='?view_admin=<?= $admin['id'] ?>'"><strong><?= $dept ?> <?= $letter ?></strong><p><?= $admin['name'] ?></p><p><?= $assignedCount ?> <?= t('users', 'usuarios') ?></p></div>
                <?php $letter++; endwhile; else: 
                    $deptUsers = $conn->query("SELECT COUNT(*) as c FROM users WHERE department = '$dept' AND role = 'user'")->fetch_assoc()['c'];
                ?>
                <div class="dept-card"><strong><?= $dept ?></strong><p><?= $deptUsers ?> <?= t('users', 'usuarios') ?>, <?= $deptAdminCount ?> admins</p></div>
                <?php endif; endforeach; ?>
            </div>
        </section>
        <!-- USER MANAGEMENT SECTION --->
        <section id="users-section" class="content-section">
            <div class="section-header"><h2><?= t('User Management', 'Usuarios') ?></h2><input type="text" id="userSearch" placeholder="<?= t('Search...', 'Buscar...') ?>" onkeyup="filterTable('usersTable', this.value)"></div>
            <div class="card">
                <table id="usersTable">
                    <thead><tr><th><?= t('Name', 'Nombre') ?></th><th><?= t('Email', 'Correo') ?></th><th><?= t('Dept', 'Depto') ?></th><th><?= t('Sub-Dept', 'Sub-Depto') ?></th><th><?= t('Status', 'Estado') ?></th><th><?= t('Actions', 'Acciones') ?></th></tr></thead>
                    <tbody>
                        <?php $allUsers->data_seek(0); while ($user = $allUsers->fetch_assoc()): ?>
                        <tr class="<?= $user['is_active'] == 0 ? 'inactive' : '' ?>">
                            <td><?= htmlspecialchars($user['name']) ?></td>

...
if (!isset($_SESSION['email']) || $_SESSION['role'] !== 'admin' || $_SESSION['admin_type'] !== 'regular') 
    {
...
function t($en, $es)
    { 
        global $lang; return $lang === 'es' ? $es : $en; 
    }
    ...
    <body>
        <!-- NAVIGATION --->
            <nav>
                ...
                <!-- TRANSLATES PLANEL TEXT --->
                <a class="nav-item active" onclick="showSection('dashboard')">
                    <?= t('Dashboard', 'Panel') ?>
                </a>

                <a class="nav-item" onclick="showSection('users')">
                    <?= t('My Users', 'Mis Usuarios') ?>
                </a>

                <a class="nav-item" onclick="showSection('incidents')">
                    <?= t('Incident Reports', 'Incidentes') ?>
                </a>

                <a class="nav-item" onclick="showSection('certificates')">
            ...
            <main class="main">
                <div class="main-top">
                    <h1>
                        ...
                        <span class="highlight">
                            <?= htmlspecialchars($currentAdmin['first_name']) ?>
                        </span>
                    </h1>
                </div>
                ...
                <section id="dashboard-section" class="content-section active">
                    ...
                    <div class="info-box">
                        <?= t('You can only manage your assigned users.', 'Solo puede administrar sus usuarios asignados.') ?>
                    </div>

                    <div class="stats-grid">
                        <div class="stat-card">
                            <h3><?= $totalUsers ?></h3>
                            <p><?= t('Assigned Users', 'Usuarios') ?></p>
                        </div>

                        <div class="stat-card">
                            <h3><?= $myIncidents->num_rows ?></h3>
                            <p><?= t('Incidents', 'Incidentes') ?></p>
                        </div>

                        <div class="stat-card">
                            <h3><?= $myCerts->num_rows ?></h3>
                ...
                <section id="users-section" class="content-section">
                    ...
                    <div class="section-header">
                        <h2><?= t('My Assigned Users', 'Mis Usuarios') ?></h2>
                                ...
                                <tr class="<?= $user['is_active'] == 0 ? 'inactive' : '' ?>">
                                    ...
                                    <td>
                                        <span class="status-badge status-<?= $user['is_active'] == 1 ? 'active' : 'inactive' ?>">
                                            <?= $user['is_active'] == 1 ? t('Active', 'Activo') : t('Inactive', 'Inactivo') ?>
                                        </span>
                                    </td>

                                    <td>
                                        <button class="btn" onclick='openEditUser(<?= json_encode($user) ?>)'>
                                            <?= t('Edit', 'Editar') ?>
                                        </button>
                                    </td>
                        ...
                        <div class="card">
                            ...
                            <div class="incident-item">
                                ...
                                <p>
                                    ...
                                    <?= t('Date', 'Fecha') ?>: <?= $inc['incident_date'] ?>
                                </p>

                                <?php if ($inc['notes']): ?>
                                    <p class="notes"><?= t('Notes', 'Notas') ?>: 
                ...
                <section id="certificates-section" class="content-section">
                    ...
                    <div class="section-header">
                        ...
                        <button class="btn" onclick="openModal('certModal')">
                            <?= t('Upload', 'Subir') ?>
                        </button>
                    </div>
                    ...
                    <div class="card">
                        <div class="cert-grid">
                            <?php $myCerts->data_seek(0); 
                            while ($cert = $myCerts->fetch_assoc()): $expiry = new DateTime($cert['expiry_date']); 
                            $today = new DateTime(); 
                            $diff = $today->diff($expiry)->days; 
                            $isExpired = $today > $expiry; $isExpiring = !$isExpired && $diff <= 30; 
                            ...
                            <div class="cert-card <?= $statusClass ?>">
                                ...
                                <span class="cert-status">
                                    <?= $isExpired ? t('Expired', 'Vencido') : 
                                    ($isExpiring ? t('Expiring', 'Por vencer') : 
                                    t('Valid', 'Valido')) ?>
                                </span>
                    ...
                    <div class="card">
                        <div class="profile-grid">
                            ...
                            <div class="profile-item">
                                ...
                                <?= htmlspecialchars($currentAdmin['middle_name'] ?? '') ?>
                            </div>

                            <div class="profile-item">
                                <strong><?= t('Last Name', 'Apellido') ?></strong>
                                <?= htmlspecialchars($currentAdmin['last_name']) ?>
                            </div>
                            ...
                            <div class="profile-item">
                                ...
                                <?= htmlspecialchars($currentAdmin['phone'] ?? 'N/A') ?>
                            </div>

                            <div class="profile-item">
                                <strong><?= t('Department', 'Depto') ?></strong>
                                <?= htmlspecialchars($currentAdmin['department'] ?? 'N/A') ?>
                            </div>
                ...
                <div class="modal-content">
                    ...
                    <form id="editUserForm" onsubmit="saveUserEdit(event)">
                        ...
                        <div class="form-group password-section">
                            ...
                            <input type="password" id="editPassword" placeholder="<?= t('Enter new password...', 'Nueva contrasena...') ?>">
                        </div>

                        <div class="form-actions">
                            <button type="submit" class="btn"><?= t('Save', 'Guardar') ?></button>
                            <button type="button" class="btn btn-secondary" onclick="closeModal('editUserModal')"><?= t('Cancel', 'Cancelar') ?></button>
                        </div>
                    </form>
                </div>
            </div>
                    ...
                    <form id="incidentForm" onsubmit="submitIncident(event)">
                        ...
                        <div class="form-group">
                            ...
                            <select id="incidentUserId" required>
                                <option value=""></option>
                                <?php $assignedUsers->data_seek(0); 
                                    while ($u = $assignedUsers->fetch_assoc()): ?>
                                    <option value="<?= $u['id'] ?>">
                    ...
                    <form id="profileForm" onsubmit="saveProfile(event)">
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <?= htmlspecialchars($currentAdmin['first_name']) ?>" required>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <?= htmlspecialchars($currentAdmin['last_name']) ?>" required>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <?= htmlspecialchars($currentAdmin['email']) ?>" required>
                        </div>

                        <div class="form-group">
                            <label><?= t('Phone', 'Telefono') ?></label>
                            <input type="text" id="profilePhone" value="
                            <?= htmlspecialchars($currentAdmin['phone'] ?? '') ?>">
                        </div>
